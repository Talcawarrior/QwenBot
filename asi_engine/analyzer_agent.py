...
class AnalyzerAgent:
    ...
    def analyze_results(self, run_round: int, hypothesis: str, 
                        backtest_results: dict, parameters: dict) -> CognitionNode:
        ...
        if roi_improved and brier_improved:
            insight = (
                f"SUCCESS: Proposed adjustments succeeded in both accuracy (Brier improved from {best_node.brier_score:.4f} "
                f"to {brier:.4f}) and profitability (ROI increased from {best_node.roi:.2f}% to {roi:.2f}%). "
                f"Shifting weights towards superior global meteorological models (like GFS/ECMWF) "
                f"reduced the impact of regional forecasting noise and sharpened the Kelly sizing."
            )
        elif roi_improved:
            insight = (
            ...
            insight = (
                f"REJECTED: Proposed weights failed to improve both Brier score ({brier:.4f} vs best {best_node.brier_score:.4f}) "
                f"and ROI ({roi:.2f}% vs best {best_node.roi:.2f}%). This indicates that trimming regional meteorological feeds "
                f"or lowering the min_edge below safety thresholds exposed the portfolio to low-probability tail-events "
                f"or over-aggression."
            )
